...
The build artifacts will be stored in the project folders. The default for most projects is `debug`.

### Tests

Run this command in terminal to execute the tests:
...
This project uses [golangci-lint](https://golangci-lint.run/) for code linting.

> ⚡ The code analysis can be configured via [.golangci.yaml](.golangci.yaml) file in the project root folder.

The code-style analysis is enabled on the build from the command-line and in IDE. To lint all modules by running:
...
This project uses [go fmt](https://pkg.go.dev/fmt) for code formatting.

To format all modules, run this command in terminal:
